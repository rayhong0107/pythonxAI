...
for img in files:
    if img[:-4] not in st.session_state.products:
        st.session_state.products[img[:-4]]={'image':f'{file_path}/{img}', "price":10, "stock": 10}
...
with col1:
    selected_products=st.selectbox("選擇商品", st.session_state.products.keys())
with col2:
    add_number=st.number_input("新增庫存數量",min_value=1, max_value=100,value=1,step=1)

if st.button("新增庫存"):
    st.session_state.products[selected_products]["stock"]+=add_number
    st.success(f'已新增{add_number}個{selected_products}庫存')
    time.sleep(.5)
    st.rerun()
...
#     st.write(f"{st.session_state.products[nam]}:{st.session_state.products[selected_products]["stock"]}件")
for name, detail in st.session_state.products.items():
    st.write(f"{name}:{st.session_state.products[selected_products]["stock"]}件")